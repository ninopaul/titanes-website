...
export default function Portfolio() {
    ...
    const fetchPortafolio = async () => {
      try {
        ...
        if (res.success && Array.isArray(res.data)) {
          // Show destacado/mostrar_en_inicio first, then others, limit to 12
          const sorted = [...res.data].sort((a, b) => {
            if (a.destacado && !b.destacado) return -1
            if (!a.destacado && b.destacado) return 1
            if (a.mostrar_en_inicio && !b.mostrar_en_inicio) return -1
            if (!a.mostrar_en_inicio && b.mostrar_en_inicio) return 1
            return a.orden - b.orden
          })
          const homeItems = sorted.slice(0, 12)
          setItems(homeItems)

          // Extract unique categories
          const cats = ['Todos', ...new Set(homeItems.map(i => i.categoria).filter(Boolean))]
          setCategories(cats)
        }
      } catch (err) {
        console.error('Error fetching portafolio:', err)
      } finally {
        setLoading(false)
      }
    }
  ...
  // Aspect ratio based on index position for visual variety
  const getAspect = (index: number): string => {
    const pattern = ['tall', 'wide', 'square', 'tall', 'wide', 'square', 'wide', 'tall', 'square', 'wide', 'tall', 'square']
      ...
      <div className="relative max-w-7xl mx-auto px-6 lg:px-8">
        ...
        {!loading && items.length === 0 && (
          <div className="text-center py-20">
            <p className="text-[#8A8A8A] text-lg">Próximamente nuestros proyectos destacados</p>
          </div>
        )}
